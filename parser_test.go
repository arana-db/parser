...
'`},
		...
		// for optimize table mode.
		{"OPTIMIZE TABLE", false, ""},
		{"OPTIMIZE TABLE t", true, "OPTIMIZE TABLE `t`"},
		{"OPTIMIZE TABLE t", true, "OPTIMIZE TABLE `t`"},
		{"OPTIMIZE TABLE t1,t2", true, "OPTIMIZE TABLE `t1`, `t2`"},
		{"OPTIMIZE NO_WRITE_TO_BINLOG TABLE t1,t2", true, "OPTIMIZE NO_WRITE_TO_BINLOG TABLE `t1`, `t2`"},
		{"OPTIMIZE LOCAL TABLE t1,t2", true, "OPTIMIZE NO_WRITE_TO_BINLOG TABLE `t1`, `t2`"},
...
func TestDBAStmt(t *testing.T) {
	table := []testCase{
		...
		// for show database rules
		{"show databaserules from xxx", true, "SHOW DATABASERULES FROM `xxx`"},
		{"show databaserules from xxx.yyy", true, "SHOW DATABASERULES FROM `xxx`.`yyy`"},
